...
public class Waits {
	...
	private FluentWait<WebDriver> waitDriver(Duration timeoutOverride) {
		Duration t = (timeoutOverride == null || timeoutOverride.isZero() || timeoutOverride.isNegative())
				? timeout
				: timeoutOverride;

		return new WebDriverWait(driver, t).pollingEvery(pollEvery).ignoring(StaleElementReferenceException.class)
	...
	public boolean textToBePresentIn(By locator, String text) {
		return Boolean.TRUE
				.equals(waitDriver().until(ExpectedConditions.textToBePresentInElementLocated(locator, text)));
	}

	public boolean checkIfElementIsInvisible(By locator) {
		return Boolean.TRUE.equals(waitDriver().until(ExpectedConditions.invisibilityOfElementLocated(locator)));
	}
